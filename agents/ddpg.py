...
def train_eval(
    ...
    with tf.compat.v2.summary.record_if(lambda: tf.math.equal(global_step % summary_interval, 0)):
        ...
        env_load_fn = lambda name, ds: suite_gym.load(name,
                                                      gym_kwargs={'data_store': ds, **gym_kwargs})

        if num_parallel_environments > 1:
            tf_env = tf_py_environment.TFPyEnvironment(
                parallel_py_environment.ParallelPyEnvironment(
                    [lambda: env_load_fn(env_name, ds_train)] * num_parallel_environments))
        else:
            tf_env = tf_py_environment.TFPyEnvironment(env_load_fn(env_name, ds_train))
